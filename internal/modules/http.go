...
func RegisterHTTP(vm *goja.Runtime, queueJob func(func()), incrementTasks func(), decrementTasks func()) {
	httpModule := vm.NewObject()

	httpModule.Set("createServer", func(call goja.FunctionCall) goja.Value {
		...
		serverObj.Set("listen", func(call goja.FunctionCall) goja.Value {
			...
			if len(call.Arguments) > 1 && !goja.IsUndefined(call.Arguments[1]) && !goja.IsNull(call.Arguments[1]) {
				...
				if hVal := opts.Get("host"); hVal != nil && !goja.IsUndefined(hVal) && !goja.IsNull(hVal) {
					host = hVal.String()
				}
			}
			...
			mux.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
				...
				queueJob(func() {
					...
					reqObj.Set("httpVersion", req.Proto)
					...
					resObj.Set("setHeader", func(name string, value string) goja.Value {
						headers := resObj.Get("_headers").ToObject(vm)
						headers.Set(name, value)
						return resObj
					})

					resObj.Set("writeHead", func(statusCode int, headers ...goja.Value) goja.Value {
						resObj.Set("_statusCode", statusCode)
						if len(headers) > 0 && !goja.IsUndefined(headers[0]) && !goja.IsNull(headers[0]) {
							h := headers[0].ToObject(vm)
							for _, key := range h.Keys() {
								resObj.Get("_headers").ToObject(vm).Set(key, h.Get(key).String())
							}
						}
						return resObj
					})
				...
				resObj.Set("write", func(data string) goja.Value {
					...
					resObj.Set("end", func(data ...goja.Value) goja.Value {
						if len(data) > 0 && !goja.IsUndefined(data[0]) && !goja.IsNull(data[0]) {
							current := resObj.Get("_body").String()
					...
					resObj.Set("status", func(code int) goja.Value {
						resObj.Set("_statusCode", code)
						return resObj
					})
					...
					queueJob(func() {
						...
						if onErr != nil && !goja.IsUndefined(onErr) && !goja.IsNull(onErr) {
							if cb, ok := goja.AssertFunction(onErr); ok {
								cb(goja.Undefined(), vm.ToValue(err.Error()))
							}
						}
					})
				}
	...
	httpModule.Set("request", func(call goja.FunctionCall) goja.Value {
		if len(call.Arguments) < 1 {
			panic(vm.ToValue("http.request requires at least 1 argument"))
		}
		...
		if optsVal != nil && !goja.IsUndefined(optsVal) && !goja.IsNull(optsVal) {
			if optsObj, ok := optsVal.Export().(map[string]interface{}); ok {
				if uVal, ok := optsObj["url"].(string); ok {
					...
					url = fmt.Sprintf("http://%s%s%s", hVal, port, path)
				}
	...
	httpModule.Set("get", func(call goja.FunctionCall) goja.Value {
		if len(call.Arguments) < 1 {
			panic(vm.ToValue("http.get requires at least 1 argument"))
		}
